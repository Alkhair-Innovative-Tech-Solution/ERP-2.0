...
class PrincipalViewSet(viewsets.ModelViewSet):
    ...
    def form_options(self, request):
        ...
        # Try to overlay org-specific options from DB if available
        try:
            from students.models import FormOption
            org = getattr(request.user, 'organization', None)
            qs = FormOption.objects.filter(is_active=True)
            qs = qs.filter(organization=org) if org else qs.filter(organization__isnull=True)
            db_options = {}
            for opt in qs:
                db_options.setdefault(opt.category, []).append({'value': opt.value, 'label': opt.label})
            for cat, items in db_options.items():
                if items:
                    defaults[cat] = items
        except Exception:
            pass
