  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 429) {
        const parsed = (() => { try { return JSON.parse(err.response || '{}'); } catch { return null; } })();
        if (parsed?.lockout_seconds) {
          setLockoutSeconds(parsed.lockout_seconds);
          toast.error('Too many failed attempts. Your account has been temporarily locked.');
        } else {
          setLockoutSeconds(60);
          toast.error('Too many requests. Please wait before trying again.');
        }
      } else if (err instanceof ApiError && err.status === 403) {
        toast.error(err.message || 'Your organization is inactive. Please contact your administrator.');
      } else if (err instanceof ApiError && err.status === 401) {
        const remaining = err.response ? (() => { try { return JSON.parse(err.response).attempts_remaining; } catch { return null; } })() : null;
        toast.error(remaining != null
      ...
      <div className="flex-1 bg-[#f8fafc] flex items-center justify-center p-6 sm:p-12 relative">
        ...
        <div className={`w-full max-w-[500px] bg-white/50 backdrop-blur-[80px] rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] border border-white/40 p-8 sm:p-12 relative z-10 transition-all duration-1000 ${animate ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
          ...
          <div className="grid grid-cols-1 grid-rows-1 items-center overflow-hidden">
            ...
            {/* Success Message (inline card — errors go to toast) */}
            {error?.type === 'success' && (
              <div className="mb-6 p-4 rounded-2xl border flex items-start gap-3 animate-in fade-in slide-in-from-top-4 duration-300 bg-emerald-50 border-emerald-200 text-emerald-800">
