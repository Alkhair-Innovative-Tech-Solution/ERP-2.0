...
import { Result, Student, SubjectMark, getStudentMonthlyAttendance } from '@/lib/api';
...
export function ReportCard({ student, results, activeMonth, availableMonths, className, retestResults = [] }: ReportCardProps) {
    const months = availableMonths && availableMonths.length > 0 ? availableMonths : DEFAULT_MONTHS;

    // Live attendance state — fetched from attendance service
    const [liveAttendance, setLiveAttendance] = useState<{ days_present: number; total_working_days: number } | null>(null);

    useEffect(() => {
        if (!student?.id || !activeMonth) { setLiveAttendance(null); return; }

        const monthlyRes = results.find((r: any) => r.exam_type === 'monthly' &&
            (r.month?.toLowerCase() === activeMonth.toLowerCase()));
        const acYear = (monthlyRes as any)?.academic_year || (() => {
            const now = new Date();
        ...
        getStudentMonthlyAttendance(student.id, activeMonth, acYear)
            .then((data: any) => setLiveAttendance(data))
            .catch(() => setLiveAttendance(null));
    }, [student?.id, activeMonth]);
    ...
    if (activeMonth) {
        return (
            <div className="min-h-screen flex items-start justify-center pt-6 pb-6 print:block print:m-0 print:p-0 print:min-h-0 print:w-full">
                <div className={`report-root relative bg-white p-8 w-full max-w-[210mm] flex flex-col mx-auto text-black print:p-6 print:max-w-full print:w-full print:mx-0 print:!bg-white ${className}`}>
                    <style dangerouslySetInnerHTML={{
                        ...
                        @media print {
                            ...
                            * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
                            ...
                            .report-root table { font-size: 11px !important; }
                            .report-root td, .report-root th { padding: 8px 10px !important; line-height: 1.4 !important; }
                            .report-root h1 { font-size: 18px !important; }
                            .report-root h3 { font-size: 11px !important; }
                            .report-root .text-3xl { font-size: 18px !important; }
                            .report-root .text-sm { font-size: 11px !important; }
                            .report-root .mb-8 { margin-bottom: 10px !important; }
                            .report-root .mb-6 { margin-bottom: 8px !important; }
                            .report-root .mb-10 { margin-bottom: 10px !important; }
                            .report-root .p-8 { padding: 8px !important; }
                            .report-root .p-4 { padding: 8px !important; }
                            .report-root .gap-8 { gap: 12px !important; }
                            .report-root .gap-6 { gap: 10px !important; }
                            .report-root .mt-6 { margin-top: 10px !important; }
                            .report-root .mt-2 { margin-top: 4px !important; }
                            .report-root .pb-6 { padding-bottom: 6px !important; }
                            .report-root .px-24 { padding-left: 80px !important; padding-right: 80px !important; }
                            .report-root .min-h-\\[80px\\] { min-height: 60px !important; }
                            .report-root .h-12 { height: 42px !important; }
                            .report-root .h-24, .report-root .print\\:h-20 { height: 56px !important; width: 56px !important; }
                    ...
                    <div className="mb-10 overflow-hidden rounded-lg border border-gray-200">
                        <Table>
                            ...
                            <TableBody>
                                {(() => {
                                    // months comes from the outer scope (dynamic — availableMonths prop or default)
                    ...
                    {/* Attendance Summary and Teacher's Remarks — flex-1 so this section fills remaining A4 space in print */}
                    <div className="remarks-grow flex-1 grid grid-cols-2 gap-6 mt-6 items-stretch">
                        {/* Attendance Summary — live data from attendance service */}
                        <div className="border-2 border-[#d1d5db] rounded-lg p-4 flex flex-col">
                            <h3 className="text-sm font-bold text-[#274c77] uppercase mb-3 border-b pb-2">Attendance Summary</h3>
                            <div className="space-y-2 text-sm">
                                {(() => {
                                    const present = liveAttendance?.days_present ?? null;
                                    const total = liveAttendance?.total_working_days ?? null;
                                    const absent = (present !== null && total !== null) ? total - present : null;
                                    const pct = (present !== null && total && total > 0) ? ((present / total) * 100).toFixed(1) + '%' : '-';
                                    return (<>
                                        ...
                                        <div className="flex justify-between">
                                            <span className="text-[#374151]">Working Days:</span>
                                            <span className="font-bold">{total ?? '-'}</span>
                                        </div>
                                        <div className="flex justify-between border-t pt-2 mt-1">
                                            <span className="text-[#374151]">Attendance %:</span>
                        ...
                        <div className="border-2 border-[#d1d5db] rounded-lg p-4 flex flex-col">
                            ...
                            <div className="remarks-inner flex-1 text-sm text-[#4b5563] italic min-h-[80px]">
                    ...
                    {/* Signatures */}
                    <div className="grid grid-cols-4 gap-8 mt-10 pt-4 pb-6 print:gap-4 print:pb-2 print:mt-6 print:pt-2 max-w-[95%] mx-auto w-full">
