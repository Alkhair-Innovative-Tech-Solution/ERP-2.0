...
def _publish(routing_key, payload):
    try:
        ...
        print(f'[WARN] signal publish {routing_key}: {e}')


@receiver(post_save, sender=Level)
def publish_level_event(sender, instance, created, **kwargs):
    routing_key = 'classes.level.created' if created else 'classes.level.updated'
...
def publish_grade_event(sender, instance, created, **kwargs):
    routing_key = 'classes.grade.created' if created else 'classes.grade.updated'
    _publish(routing_key, {
        'id': instance.id,
        'name': instance.name,
        'code': instance.code,
...
def publish_classroom_event(sender, instance, created, **kwargs):
    routing_key = 'classes.classroom.created' if created else 'classes.classroom.updated'
    _publish(routing_key, {
        'id': instance.id,
        'code': instance.code,
        ...
        'grade_code': instance.grade.code if instance.grade else None,
        'level_id': instance.grade.level_id if instance.grade else None,
        'campus_id': instance.grade.campus_id if instance.grade else None,
        'organization_id': instance.organization_id,
    })
