...
def publish_student_upserted(instance):
    """Publish a student.upserted event so attendance/other services can sync.

    Reusable so bulk imports (which use bulk_create and bypass post_save
    signals) can publish events explicitly for every created student.
    """
    try:
        ...
        payload = {
            ...
            'gr_no': instance.gr_no,
            'father_name': instance.father_name,
            'father_cnic': instance.father_cnic,
        ...
        publish_event('student.upserted', payload)
    except Exception as e:
        logger.warning("Failed to publish student.upserted: %s", e)


@receiver(post_save, sender=Student)
def publish_student_event(sender, instance, **kwargs):
    """Publish student upsert event so attendance-service can sync."""
    publish_student_upserted(instance)
