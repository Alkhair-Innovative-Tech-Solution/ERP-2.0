...
OPTIONAL_FIELDS = [
    ...
    'assigned_classroom', 'assigned_classrooms', 'subject_assignments', 'coordinators'
]
...
SAMPLE_ROW = {
    ...
    'assigned_classroom': 'Grade I - A, Grade II - B',
...
def import_teachers_from_csv(path, user):
    ...
    org_id = getattr(user, 'org_id', None) or getattr(org, 'pk', None)
    ...
    all_subjects = list(Subject.objects.filter(organization=org)) if Subject else []
    ...
    for idx, row in enumerate(rows, start=2):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                class_input = (row.get('assigned_classrooms') or row.get('assigned_classroom') or '').strip()
                if class_input:
