...
def _attendance_marked_classroom_ids(classroom_ids, target_date):
    """Classrooms with a submitted-or-higher attendance record on target_date.

    Attendance lives in attendance_db (attendance-service), not staff_db, so
    read it cross-DB via psycopg2. Returns an empty set on any error — the
    status screen then shows everyone as unmarked rather than crashing.
    """
    ...
    try:
        ...
        conn = psycopg2.connect(
            host=os.environ.get('ATTENDANCE_DB_HOST', 'postgres-attendance'),
            dbname=os.environ.get('ATTENDANCE_DB_NAME', 'attendance_db'),
            user=os.environ.get('ATTENDANCE_DB_USER', 'attendance_user'),
            password=os.environ.get('ATTENDANCE_DB_PASSWORD', 'attendance_pass'),
            connect_timeout=5,
        )
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT DISTINCT classroom_id FROM attendance_attendance "
                    "WHERE classroom_id = ANY(%s) AND date = %s "
                    "AND status IN ('submitted', 'under_review', 'approved') "
                    "AND is_deleted = false",
                    (list(classroom_ids), target_date),
                )
...
class CoordinatorViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if hasattr(user, 'is_principal') and user.is_principal():
            # _TokenUser has campus_id/org_id (no .campus/.organization objects)
    ...
    def attendance_status(self, request, pk=None):
        ...
        marked_classroom_ids = _attendance_marked_classroom_ids(
            list(classrooms.values_list('id', flat=True)), target_date
        )
    ...
    def remind_attendance(self, request, pk=None):
        ...
        marked_classroom_ids = _attendance_marked_classroom_ids(
            [c.id for c in classrooms], target_date
        )
    ...
    @decorators.action(detail=False, methods=['get'], url_path='my-teachers')
    def my_teachers(self, request):
        """Get teachers for the currently logged-in coordinator (no explicit ID needed)."""
        coordinator = Coordinator.get_for_user(request.user)
        if not coordinator:
            return response.Response({'error': 'Coordinator profile not found'}, status=404)
        # Reuse the detail teachers logic
        request.META['_coordinator_pk'] = coordinator.pk
        teachers = Teacher.objects.filter(
            assigned_coordinators=coordinator,
            is_currently_active=True
        ).select_related('current_campus', 'assigned_classroom', 'assigned_classroom__grade__level') \
         .prefetch_related('assigned_coordinators', 'assigned_classrooms', 'assigned_classrooms__grade__level', 'classroom_set', 'classroom_set__grade__level')
        ...
        if not teachers.exists():
            ...
            if managed_levels:
                from classes.models import ClassRoom
                classrooms = ClassRoom.objects.filter(grade__level__in=managed_levels).select_related('class_teacher')
                teacher_ids = set()
                for classroom in classrooms:
                    if classroom.class_teacher_id:
                        teacher_ids.add(classroom.class_teacher_id)
                if teacher_ids:
                    teachers = Teacher.objects.filter(id__in=teacher_ids, is_currently_active=True) \
                        .select_related('current_campus', 'assigned_classroom', 'assigned_classroom__grade__level') \
                        .prefetch_related('assigned_coordinators', 'assigned_classrooms', 'assigned_classrooms__grade__level', 'classroom_set', 'classroom_set__grade__level')

        from teachers.serializers import TeacherSerializer
        serializer = TeacherSerializer(teachers, many=True, context={'request': request})
        return response.Response({'coordinator_id': coordinator.id, 'teachers': serializer.data})

    @decorators.action(detail=False, methods=['get'], url_path='me')
    def me(self, request):
        """Return the current coordinator's profile (id, name, campus, level, shift)."""
        coordinator = Coordinator.get_for_user(request.user)
        if not coordinator:
            return response.Response({'error': 'Coordinator profile not found'}, status=404)
        from .serializers import CoordinatorSerializer
        return response.Response(CoordinatorSerializer(coordinator, context={'request': request}).data)
