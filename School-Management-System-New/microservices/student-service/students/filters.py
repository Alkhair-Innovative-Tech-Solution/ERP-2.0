...
class StudentFilter(django_filters.FilterSet):
    ...
    # Accept multiple genders (comma-separated)
    gender = django_filters.CharFilter(
        method='filter_gender',
        help_text="Filter by gender(s), comma-separated"
    )
    
    shift = django_filters.CharFilter(
        method='filter_shift',
        help_text="Filter by shift(s), comma-separated"
    )
    ...
    # Date range filters
    enrollment_year = django_filters.CharFilter(
        method='filter_enrollment_year',
        help_text="Filter by enrollment year(s), comma-separated"
    )
    ...
    def filter_gender(self, queryset, name, value):
        if not value:
            return queryset
        genders = [g.strip().lower() for g in value.split(',')]
        return queryset.filter(gender__in=genders)

    def filter_enrollment_year(self, queryset, name, value):
        if not value:
            return queryset
        years = [y.strip() for y in value.split(',') if y.strip().isdigit()]
        if not years:
            return queryset
        return queryset.filter(enrollment_year__in=years)
    ...
    def filter_shift(self, queryset, name, value):
        ...
        shifts = [s.strip().lower() for s in value.split(',')]
        query = Q()
        for shift_value in shifts:
            query |= Q(classroom__isnull=False, classroom__shift__iexact=shift_value) | Q(classroom__isnull=True, shift__iexact=shift_value)
            
        return queryset.filter(query)
    
    def filter_section(self, queryset, name, value):
        """Filter by section - check both student.section and classroom.section. Accepts comma-separated."""
        if not value:
            return queryset
        
        sections = [s.strip().upper() for s in value.split(',')]
        query = Q()
        for section_value in sections:
            query |= Q(section__iexact=section_value) | Q(classroom__section__iexact=section_value)
            
        return queryset.filter(query)
    
    def filter_current_grade(self, queryset, name, value):
        """Filter by grade - check both student.current_grade and classroom.grade.name with precise matching. Accepts comma-separated values."""
        if not value:
            return queryset
        
        grades_list = [g.strip() for g in value.split(',')]
        final_query = Q()
        ...
        for grade_value in grades_list:
            ...
            final_query |= (student_grade_query | classroom_grade_query)
