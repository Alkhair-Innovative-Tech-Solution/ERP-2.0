...
export default function ResultApprovalPage() {
  ...
  const dynamicMonths = useMemo(() => {
    const startMonth = coordinatorProfile?.campus?.academic_year_start_month;
    if (!startMonth) return ALL_MONTHS;
    const startIdx = ALL_MONTHS.indexOf(startMonth);
    if (startIdx === -1) return ALL_MONTHS;
    const months: string[] = [];
    let curr = startIdx;
    for (let i = 0; i < 12; i++) {
      months.push(ALL_MONTHS[curr]);
      curr = (curr + 1) % 12;
  ...
  const monthYearMap = useMemo(() => {
    const startMonth = coordinatorProfile?.campus?.academic_year_start_month;
    if (!startMonth) return {} as Record<string, number>;
    const startIdx = ALL_MONTHS.indexOf(startMonth);
    const now = new Date();
    const currentMonthIdx = now.getMonth();
    const currentYear = now.getFullYear();
    const startYear = currentMonthIdx >= startIdx ? currentYear : currentYear - 1;
    const map: Record<string, number> = {};
    let year = startYear;
    let curr = startIdx;
    for (let i = 0; i < 12; i++) {
      map[ALL_MONTHS[curr]] = year;
      if (curr === 11) year++;
      curr = (curr + 1) % 12;
      if (curr === startIdx) break;
    }
  ...
  useEffect(() => {
    if (coordinatorProfile?.campus?.academic_year_start_month) {
      setActiveMonthTab(coordinatorProfile.campus.academic_year_start_month);
    }
  }, [coordinatorProfile]);
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-8 bg-gray-50/30 min-h-screen print:hidden">
      ...
      {/* View Modal - Student Report Card */}
      {
        showViewModal && selectedResultView && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200 overflow-y-auto">
            <Card className="w-full max-w-[215mm] max-h-[95vh] overflow-hidden flex flex-col shadow-2xl border-0 rounded-2xl">
              {/* Modal Header */}
              <div className="bg-white px-8 py-5 border-b flex justify-between items-center sticky top-0 z-10 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    onClick={() => {
                      if (reportCardData) {
                        ...
                        window.open(`/admin/report-card-preview?key=${key}`, '_blank');
                      }
                    }}
                    variant="outline"
              ...
              {/* Modal Footer - Actions for Approval */}
              {!fetchingReportCard && selectedResultView && (
                <div className="bg-white p-5 border-t flex justify-end gap-3 shrink-0 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
                  <Button
                    variant="outline"
                    size="lg"
                    onClick={() => {
