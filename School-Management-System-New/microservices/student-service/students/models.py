...
class Student(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Raise if still no classroom on creation (even if section was cleared above)
        if not self.classroom and is_create and not self.is_draft:
        ...
        # Generate student_id using org pattern if available, else legacy campus-based
        if not self.student_id and all([self.campus, self.shift, self.enrollment_year]):
            try:
                from utils.id_generator import IDGenerator
                student_id, gr_no = IDGenerator.generate_student_id(
                    self.campus, self.shift, self.enrollment_year
                )
        ...
        # Fallback gr_no if still missing after ID generation
        if self.student_id and not self.gr_no:
            parts = self.student_id.replace('/', '-').split('-')
            serial = next((p for p in reversed(parts) if p.isdigit()), None)
            self.gr_no = serial.zfill(5) if serial else self.student_id[-5:].zfill(5)
