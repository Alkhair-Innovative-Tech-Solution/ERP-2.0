...
import {
  ...
  updateClassTimetable, createTeacherTimetable, updateTeacherTimetable,
  deleteClassTimetable, deleteTeacherTimetable, bulkCreateTeacherPeriods
} from "@/lib/api";
...
export default function Page() {
  ...
  const [dragHighlightKeys, setDragHighlightKeys] = useState<Set<string>>(new Set());
  ...
  const dragHighlightRef = useRef<Map<string, { day: string; daySlot: ShiftTiming }>>(new Map());
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const commonData = {
        ...
        subject: dialogData.subjectId ? parseInt(dialogData.subjectId) : null,
      };
      ...
      if (timetableType === 'class') {
        ...
        if (dialogData.existingId) {
          await updateClassTimetable(dialogData.existingId, payload);
        } else {
        ...
        const payload = {
          ...
          classroom: dialogData.classroomId ? parseInt(dialogData.classroomId) : null,
        };

        if (dialogData.existingId) {
          await updateTeacherTimetable(dialogData.existingId, payload);
        } else {
  ...
  const handleDragHandleMouseDown = (e: React.MouseEvent, assignment: any) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    dragDataRef.current = {
      ...
      teacherId: (assignment.teacher ?? "").toString(),
      classroomId: (assignment.classroom ?? "").toString(),
      subjectName: assignment.subject_name || assignment.subject?.name || "",
    };
    dragHighlightRef.current = new Map();
    setIsDragging(true);
    setDragHighlightKeys(new Set());
  };

  const handleCellMouseEnter = (cellKey: string, day: string, daySlot: ShiftTiming, assignment: any, isBreak: boolean) => {
    if (!isDraggingRef.current || isBreak || assignment) return;
    dragHighlightRef.current.set(cellKey, { day, daySlot });
    setDragHighlightKeys(new Set(dragHighlightRef.current.keys()));
  };

  const handleMouseUp = useCallback(async () => {
    if (!isDraggingRef.current) return;

    const highlight = new Map(dragHighlightRef.current);
    ...
    if (!data || highlight.size === 0) return;

    const creates = Array.from(highlight.values()).map(({ day, daySlot }) => {
      if (timetableType === 'class') {
        // Primary: use the source assignment's classroom (most reliable)
        // Fallback: find from loaded classrooms by grade+section
        const classroomId = data.classroomId
          ? parseInt(data.classroomId)
          : classrooms.find((c: any) => c.grade === selectedGrade && c.section === selectedSection)?.id;

        return createClassTimetable({
          day: day.toLowerCase(),
          start_time: daySlot.start_time,
          end_time: daySlot.end_time,
          subject: parseInt(data.subjectId),
          teacher: data.teacherId ? parseInt(data.teacherId) : null,
          classroom: classroomId,
        });
      } else {
        return createTeacherTimetable({
          day: day.toLowerCase(),
          start_time: daySlot.start_time,
          end_time: daySlot.end_time,
          subject: parseInt(data.subjectId),
          teacher: parseInt(selectedTeacherId),
          classroom: data.classroomId ? parseInt(data.classroomId) : null,
        });
      }
    });
    ...
    if (creates.length > 0) {
      try {
        ...
        toast.success(`Assigned to ${creates.length} period${creates.length > 1 ? 's' : ''}`);
        fetchTimetable();
      } catch {
        toast.error('Some assignments failed. Check for conflicts.');
        fetchTimetable();
      }
    }
  }, [timetableType, classrooms, selectedGrade, selectedSection, selectedTeacherId]);
                    ...
                    return (
                      <tr key={baseName} className="hover:bg-[#e7ecef]/30 transition-all duration-200">
                        ...
                        {WEEK_DAYS.map((day) => {
                          ...
                          const cellKey = `${day}|||${baseName}`;
                          const isHighlighted = dragHighlightKeys.has(cellKey);

                          return (
                            <td
                              key={day}
                              className={`p-2 border-b border-r last:border-r-0 transition-colors relative ${isBreak
                                ? 'bg-slate-100/40'
                                : isHighlighted
                                  ? 'bg-blue-100/60 ring-2 ring-inset ring-[#6096ba] cursor-crosshair'
                                  : 'bg-white hover:bg-blue-50/30 cursor-pointer'
                                }`}
                              onClick={() => !isDragging && handleCellClick(day, daySlot, assignment)}
                              onMouseEnter={() => handleCellMouseEnter(cellKey, day, daySlot, assignment, !!isBreak)}
                            >
                              {isBreak ? (
                              ...
                              ) : assignment && (assignment.subject_name || assignment.subject?.name || assignment.subject) ? (
                                <div className="relative bg-gradient-to-br from-[#274c77] to-[#6096ba] text-white p-3 rounded-xl shadow-lg h-full border border-white/10 transform transition-all hover:scale-[1.02] hover:shadow-xl">
                                  <div className="font-bold text-[12px] sm:text-[13px] mb-1 leading-tight tracking-wide">
                                    {assignment.subject_name || assignment.subject?.name}
                                  </div>
                                  ...
                                  <div
                                    title="Drag to fill other periods"
                                    onMouseDown={(e) => handleDragHandleMouseDown(e, assignment)}
                                    className="absolute bottom-1 right-1 w-3 h-3 bg-white rounded-sm cursor-crosshair opacity-60 hover:opacity-100 transition-opacity border border-[#6096ba]/40 hover:border-[#274c77] hover:scale-110"
                                  />
                                </div>
                              ) : isHighlighted ? (
                                <div className="flex flex-col items-center justify-center py-4 gap-1">
                                  <div className="text-[10px] font-bold text-[#274c77] text-center leading-tight">
                                    {dragDataRef.current?.subjectName}
                                  </div>
                                  <div className="text-[9px] text-[#6096ba] font-semibold uppercase tracking-wider">Will assign</div>
                                </div>
                              ) : (
