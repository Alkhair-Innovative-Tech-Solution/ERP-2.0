    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-12 gap-4 sm:gap-5">
        ...
        <div className="xl:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col">
          ...
          <button onClick={() => router.push('/coordinator/requests')} className="mt-4 w-full h-9 text-sm font-semibold text-[#274c77] bg-[#6096ba]/10 hover:bg-[#6096ba]/20 rounded-lg transition-colors">Go to Requests Desk</button>
        </div>
      </section>
      ...
      <section>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="flex items-center gap-2 font-bold text-[#274c77]"><Activity className="h-5 w-5 text-[#6096ba]" /> Recent Activities</h3>
            <button onClick={() => router.push('/coordinator/requests')} className="text-xs font-semibold text-[#6096ba] hover:text-[#274c77]">View All</button>
          </div>
          {displayedRequests.length ? (
            <div className="space-y-1">
              {displayedRequests.map((req) => (
                <button key={req.id} onClick={() => router.push('/coordinator/requests')} className="w-full flex items-center gap-3 rounded-xl px-2 py-2.5 hover:bg-gray-50 transition-colors text-left">
                  <div className="w-9 h-9 rounded-full bg-[#6096ba]/10 flex items-center justify-center flex-shrink-0"><ClipboardList className="h-4 w-4 text-[#6096ba]" /></div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-800 truncate">{req.subject}</p>
                    <p className="text-xs text-gray-400">{req.teacher_name} · {formatDate(req.updated_at)}</p>
                  </div>
        ...
        {/* Quick Actions */}
        <div className="xl:col-span-7 bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              { label: 'Review Attendance', Icon: ClipboardCheck, route: '/coordinator/attendance-review', color: '#6096ba' },
              { label: 'Manage Students', Icon: Users, route: '/admin/students/student-list', color: '#10B981' },
              { label: 'Approve Results', Icon: FileText, route: '/coordinator/result-approval', color: '#8B5CF6' },
              { label: 'Teacher Directory', Icon: GraduationCap, route: '/admin/teachers/list', color: '#F59E0B' },
              { label: 'Timetable Management', Icon: Calendar, route: '/coordinator/time-table', color: '#274c77' },
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          ...
          {attendanceStatus.unmarked_count > 0 ? (
            ...
            <div className="flex items-center gap-2 text-sm text-[#10B981] bg-green-50 border border-green-100 rounded-xl px-4 py-3">
              <CheckCircle2 className="h-5 w-5" /> All classes have attendance marked for this date.
            </div>
          )}
        </section>
      )}
