...
class CoordinatorSerializer(serializers.ModelSerializer):
    ...
    # Use _base_manager so PK validation isn't filtered out by OrganizationManager
    # (which returns .none() when evaluated outside a request context). Org isolation
    # is enforced separately via perform_create + frontend scoping.
    level = serializers.PrimaryKeyRelatedField(
        required=False, allow_null=True, queryset=Level._base_manager.all()
    )
    campus = serializers.PrimaryKeyRelatedField(
        required=False, allow_null=True, queryset=Campus._base_manager.all()
    )
    assigned_levels = serializers.PrimaryKeyRelatedField(
        many=True, required=False, allow_empty=True, queryset=Level._base_manager.all()
    )
