...
def _notify_timetable_sync():
    """Notify timetable-service to re-sync teacher data after create/update."""
    import requests as req_lib
    url = os.environ.get('TIMETABLE_SERVICE_URL', 'http://timetable-service:8009')
    secret = os.environ.get('INTERNAL_SERVICE_SECRET', '')
    try:
        resp = req_lib.post(
            f"{url}/api/internal/sync-teachers/",
            headers={'X-Internal-Secret': secret},
            timeout=5,
        )
        if resp.status_code != 200:
            print(f"[TEACHER SYNC] timetable-service returned {resp.status_code}: {resp.text}")
    except Exception as e:
        print(f"[TEACHER SYNC] Could not reach timetable-service: {e}")
...
class TeacherViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superadmin():
            ...
            if campus:
                ...
                queryset = queryset.filter(current_campus_id=campus_id)
    ...
    def perform_create(self, serializer):
        ...
        if not user.is_superadmin():
            ...
            if org_id:
                ...
                try:
                    save_kwargs['organization'] = Organization.all_objects.get(pk=org_id)
                except Exception:
                    pass

        instance = serializer.save(**save_kwargs)
        _notify_timetable_sync()
        return instance
    ...
    def my_classes(self, request):
        ...
        try:
            ...
            # 2. Subject Teacher Assignments (timetable.Subject may not be installed)
            try:
                subject_assignments = list(TeacherSubjectAssignment.objects.filter(teacher=teacher).select_related('classroom'))
            except Exception:
                subject_assignments = []
            ...
            for sa in subject_assignments:
                try:
                    classrooms_data.append({
                        ...
                        'subject_name': str(sa.subject_id),
