...
def passed_final_last_year(student_ids, prev_year):
    ...
    pass_status='pass'. Result lives in result_db (result-service), not here —
    read it cross-DB via psycopg2, restricted to the already-scoped
    `student_ids` so this stays within the caller's campus/level scope.
    """
    if not student_ids:
        return set()
    import psycopg2
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT DISTINCT student_id FROM result_result "
                "WHERE student_id = ANY(%s) AND academic_year = %s "
                "AND exam_type = 'final' AND status = 'approved' AND pass_status = 'pass'",
                (list(student_ids), prev_year),
            )
