...
export function PrincipalEditForm({
  ...
  const validateClientSide = (): Record<string, string> => {
    ...
    if (!(editFormData.full_name ?? "").toString().trim()) errs.full_name = "Full name is required.";
    const email = (editFormData.email ?? "").toString().trim();
    if (!email) errs.email = "Email is required.";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errs.email = "Enter a valid email address (e.g. name@example.com).";
    const phone = (editFormData.contact_number ?? "").toString().replace(/\D/g, "");
    if (phone && !((phone.length === 11 && phone.startsWith("03")) || (phone.length === 12 && phone.startsWith("923")))) {
      errs.contact_number = "Must be a valid phone — 11 digits starting with 03 (e.g. 03121234567).";
    }
    const cnic = (editFormData.cnic ?? "").toString().replace(/\D/g, "");
    if (cnic && cnic.length !== 13) errs.cnic = "CNIC must be exactly 13 digits.";
    return errs;
  };
