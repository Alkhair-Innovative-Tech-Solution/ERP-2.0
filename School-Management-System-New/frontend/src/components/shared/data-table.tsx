    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden w-full max-w-full border border-gray-100">
      ...
      <div className="block sm:hidden space-y-2.5 p-2">
        {data.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`bg-white border rounded-xl shadow-sm p-3 transition-colors ${selectedIds.includes(item.id) ? 'border-[#2F6B8A]/40 bg-[#f4f7ff]' : 'border-gray-100'}`}
            >
      ...
      <div className="hidden sm:block overflow-x-auto w-full">
        <Table>
          ...
          <TableBody>
            {data.map((item) => (
              <TableRow key={item.id} className={`hover:bg-gray-50/70 text-gray-900 transition-colors ${selectedIds.includes(item.id) ? 'bg-[#f4f7ff]' : ''}`}>
                {onSelectionChange && (
