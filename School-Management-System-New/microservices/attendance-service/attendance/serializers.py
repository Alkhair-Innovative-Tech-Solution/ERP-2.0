...
from .models import Attendance, StudentAttendance, Weekend, AuditLog, BiometricDevice, BiometricEmployeeMapping, StaffAttendance, EmployeeShiftTiming
...
class StaffAttendanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'status', 'late_minutes', 'late_formatted',
            'early_departure_minutes', 'early_departure_formatted', 'source', 'device',
...
class BiometricDeviceSerializer(serializers.ModelSerializer):
    ...
    # FoxFace (cloud-pull) devices have no IP. Accept blank/null gracefully.
    ip_address = serializers.IPAddressField(required=False, allow_null=True, allow_blank=True)

    class Meta:
        model = BiometricDevice
        fields = [
            'id', 'name', 'device_type', 'ip_address', 'port', 'serial_number',
            'device_model', 'campus', 'campus_name', 'is_active',
            'last_sync', 'is_online', 'last_status_check', 'created_at', 'mapping_count',
        ]
        read_only_fields = ['id', 'created_at', 'last_sync', 'is_online', 'last_status_check']

    def validate_ip_address(self, value):
        # Normalise "" -> None so the nullable column stores NULL, not blank.
...
class BiometricEmployeeMappingSerializer(serializers.ModelSerializer):
