...
export function CoordinatorForm({
    ...
    try {
      ...
      if (userRole === 'principal') {
        ...
        if ((user as any)?.campus) {
          const principalCampus = (user as any).campus;
          setCurrentUserCampus(principalCampus);
          setCampuses([principalCampus]);
          applyShiftOptions(principalCampus);
          setFormData((prev: any) => ({ ...prev, campus: principalCampus.id }));
        } else {
  ...
  const applyShiftOptions = (campus: any) => {
    const available = campus?.shift_available || 'both'
    let opts: { value: string; label: string }[] = []
    if (available === 'morning') {
      opts = [{ value: 'morning', label: 'Morning' }]
    } else if (available === 'afternoon') {
      opts = [{ value: 'afternoon', label: 'Afternoon' }]
    } else {
      opts = [
        { value: 'morning', label: 'Morning' },
        { value: 'afternoon', label: 'Afternoon' },
    ...
    if (opts.length === 1) {
      setFormData((prev: any) => ({ ...prev, shift: opts[0].value }))
    }
  }
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    if (field === 'campus' && value) {
      ...
      applyShiftOptions(selectedCampus)
