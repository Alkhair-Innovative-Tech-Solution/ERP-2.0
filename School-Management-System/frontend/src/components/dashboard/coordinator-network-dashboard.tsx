...
function StudentList({ icon, title, rows, tone }: { icon: ReactNode; title: string; rows: any[]; tone: "top" | "fail" }) {
  return (
    <Card className="border border-gray-100">
    ...
    <div className="space-y-5">
      ...
      {!loading && intra && mode === "intra" && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <StudentList icon={<Trophy className="h-4 w-4" />} title="Top Performing Students" rows={intra.top_students || []} tone="top" />
            <StudentList icon={<AlertTriangle className="h-4 w-4" />} title="Students Needing Action" rows={intra.failing_students || []} tone="fail" />
          </div>
        </>
      )}
