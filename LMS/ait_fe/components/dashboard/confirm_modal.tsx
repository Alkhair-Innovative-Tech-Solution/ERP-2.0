  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-md z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-[90vw] md:w-[85vw] max-h-[90vh] h-[85vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-4 text-blue-600">Detailed Review</h2>

        {/* Topic Navigation */}
        <div className="flex justify-between mb-4">
          <button
            className={`px-4 py-2 rounded-md ${topicIndex === 0 ? 'bg-gray-300' : 'bg-blue-500 text-white'}`}
            disabled={topicIndex === 0}
            onClick={() => { setTopicIndex(topicIndex - 1); setQuestionIndex(0); }}
          >
            ⬅ Previous Topic
          </button>
          <h3 className="text-lg font-semibold text-blue-700">{currentTopic.subject} - {currentTopic.topic}</h3>
          <button
            className={`px-4 py-2 rounded-md ${topicIndex === topics.length - 1 ? 'bg-gray-300' : 'bg-blue-500 text-white'}`}
                  ...
                  <div key={option.id} className="relative flex flex-col">
                    <div
                      className={`border border-gray-300 rounded-lg p-4 text-left transition duration-200 ease-in-out flex justify-between w-full
                        ...
                        ${isSelected ? (isCorrect ? "bg-green-400" : "bg-red-400") : ""}
        ...
        <div className="flex justify-center mt-6">
          <button className="px-5 py-2 bg-red-500 text-white rounded-md" onClick={onCancel}>Exit</button>
        </div>
      </div>
    </div>
  );
