...
export default function StudentDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Fetch Results — use no-arg form so it hits /api/result/student/my-results/ (result-service)
        const results = await getStudentResults();
        // Response is [{overall: {...}, subject_marks: [...]}] — flatten for easy access
        const normalized = Array.isArray(results)
          ? results.map((r: any) => ({ ...(r.overall || r), subject_marks: r.subject_marks || [] }))
          : [];
        setResultsRaw(normalized);
  ...
  const latestResult = useMemo(() => {
    ...
    return [...resultsRaw].sort((a, b) => {
      // Prioritize final, then midterm, then monthly
      const typeRank: Record<string, number> = { 'final': 3, 'final_term': 3, 'midterm': 2, 'mid_term': 2, 'monthly': 1 };
