...
export default function LevelManagement({ campusId }: LevelManagementProps) {
  ...
  async function handleCoordinatorAssignment() {
    ...
    const coordId = parseInt(selectedCoordinatorId)
    const coordObj = availableCoordinators.find(c => c.id === coordId)
    const coordName = coordObj ? coordObj.full_name : ''
    const coordCode = coordObj ? (coordObj.employee_code || '') : ''

    setAssigning(true)
    try {
      // 1. Update campus-service Level record (stores coordinator_id + name + code)
      await assignCoordinatorToLevel(selectedLevel.id, coordId, coordName, coordCode)
      // 2. Update staff-service coordinator record (sets coordinator.level)
      try {
        const { apiPatch } = await import('@/lib/api')
        await apiPatch(`/api/coordinators/${coordId}/`, { level: selectedLevel.id })
      } catch { /* best-effort — campus-service already updated */ }
                  ...
                  {level.coordinator_name ? (
                    <div className="text-xs text-gray-700 flex items-center justify-between">
                      <div>
                        ...
                        {level.coordinator_code ? <span className="text-gray-500"> ({level.coordinator_code})</span> : null}
                      </div>
                      <Button 
                        size="sm" 
                        variant="ghost" 
                      ...
                      {level.coordinator_name ? (
                        <div className="flex items-center gap-3">
                          <div className="flex flex-col">
                            ...
                            {level.coordinator_code ? <span className="text-[10px] text-gray-500">({level.coordinator_code})</span> : null}
                          </div>
                          <Button 
                            size="sm" 
                            variant="outline" 
