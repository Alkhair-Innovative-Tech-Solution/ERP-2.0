...
class _CoordinatorProxy:
    """Lightweight coordinator data holder populated from staff_db when local lookup fails."""
    __slots__ = ('id', 'shift', 'level_id', 'campus_id', 'is_currently_active', '_level_ids', '_campus', 'email', 'employee_code')

    def __init__(self, id, shift, level_id, campus_id, is_currently_active, level_ids=None, email=None, employee_code=None):
        self.id = id
        self.shift = shift
        self.level_id = level_id
        self.campus_id = campus_id
        self.is_currently_active = is_currently_active
        self._level_ids = level_ids or ([level_id] if level_id else [])
        self.email = email or ''
        self.employee_code = employee_code or ''

    @property
    def full_name(self):
        return self.employee_code or self.email or f'Coordinator#{self.id}'

    # Mimic ORM interface used across views
    def assigned_levels_ids(self):
    ...
    def level(self):
        ...
        try:
            return Level._base_manager.get(pk=self.level_id)
        except Exception:
            return None
    ...
    def campus(self):
        ...
        try:
            ...
            return Campus.objects.get(pk=self.campus_id)
        except Exception:
            return None

    class _LevelSet:
        def __init__(self, level_ids):
            self._ids = level_ids
        def exists(self):
            return bool(self._ids)
        def all(self):
            from classes.models import Level
            return Level._base_manager.filter(pk__in=self._ids)
        def values_list(self, *args, **kwargs):
            return self._ids

    @property
    def assigned_levels(self):
        return self._LevelSet(self._level_ids)


def _resolve_coordinator(user):
    """
    Resolve coordinator for a request user.
    Tries local attendance_db first; falls back to staff_db when not found.
    Returns Coordinator ORM instance OR _CoordinatorProxy OR None.
    """
    from coordinator.models import Coordinator
    obj = Coordinator.get_for_user(user)
    if obj:
        return obj

    # Fallback: query staff_db directly
    import os
    try:
        import psycopg2
        ...
        with conn.cursor() as cur:
            ...
            for val, col in [(username, 'employee_code'), (email, 'email')]:
                if val and not row:
                    cur.execute(
                        f"SELECT id, shift, level_id, campus_id, is_currently_active, employee_code, email FROM coordinator_coordinator WHERE {col}=%s AND is_deleted=false LIMIT 1",
                        (val,)
                    )
                    row = cur.fetchone()
            if not row:
                conn.close()
                return None
            coord_id, shift, level_id, campus_id, is_active, emp_code, coord_email = row
        ...
        conn.close()
        return _CoordinatorProxy(coord_id, shift, level_id, campus_id, is_active, level_ids, email=coord_email, employee_code=emp_code)
    except Exception:
        return None


def _get_coordinator_teacher_identifiers(user, coordinator_obj):
    """
    Return (emp_codes, emails) lists for teachers assigned to this coordinator.
    Tries ORM M2M first; falls back to a direct staff_db query.
    """
    emp_codes, emails = [], []

    # 1. Try ORM M2M (works after sync_master_data populates the M2M table)
    if coordinator_obj and hasattr(coordinator_obj, 'assigned_teachers'):
        try:
            assigned = coordinator_obj.assigned_teachers.filter(is_currently_active=True)
            emp_codes = [t.employee_code for t in assigned if t.employee_code]
            emails    = [t.email for t in assigned if t.email]
        except Exception:
            pass

    # 2. If M2M is empty (not yet synced), fall back to direct staff_db query
    if not emp_codes and not emails:
        import os, psycopg2
        username = getattr(user, 'username', None)
        email    = getattr(user, 'email', None)
        try:
            ...
            with conn.cursor() as cur:
                ...
                coord_id = None
                for val, col in [(username, 'employee_code'), (email, 'email')]:
                    if val and not coord_id:
                        cur.execute(
                            f"SELECT id FROM coordinator_coordinator WHERE {col}=%s AND is_deleted=false LIMIT 1",
                            (val,)
                        )
                        row = cur.fetchone()
                        if row:
                ...
                if coord_id:
                    cur.execute(
                        "SELECT t.employee_code, t.email FROM teachers_teacher t "
                        "JOIN teachers_teacher_assigned_coordinators m ON m.teacher_id = t.id "
                        "WHERE m.coordinator_id = %s AND t.is_currently_active = true",
                        (coord_id,)
                    )
                    for row in cur.fetchall():
                        if row[0]: emp_codes.append(row[0])
                        if row[1]: emails.append(row[1])
            conn.close()
        except Exception:
            pass

    return emp_codes, emails
...
from .serializers import (
    ...
    BiometricEmployeeMappingSerializer,
...
def get_class_students(request, classroom_id):
    ...
    if user.is_teacher():
        try:
            ...
            # Teacher not synced to this service — verify org match only
    ...
    try:
        ...
        if user.is_teacher():
            try:
                ...
                # Teacher not synced — verify org match only
                org_id = getattr(user, 'org_id', None)
                classroom_org_id = getattr(getattr(classroom, 'organization', None), 'pk', None)
                if org_id and classroom_org_id and org_id != classroom_org_id:
                    return Response({'error': 'Access denied'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if coordinator and coordinator.is_currently_active:
            ...
            try:
                ...
                with conn.cursor() as cur:
                    ...
                    if username:
                        cur.execute(
                            "SELECT shift, level_id, is_currently_active FROM coordinator_coordinator WHERE employee_code=%s AND is_deleted=false LIMIT 1",
                            (username,)
                        )
                        row = cur.fetchone()
                    if not row and email:
                        cur.execute(
                            "SELECT shift, level_id, is_currently_active FROM coordinator_coordinator WHERE email=%s AND is_deleted=false LIMIT 1",
                            (email,)
                        )
                        row = cur.fetchone()

                    if row:
                        shift, level_id, is_active = row
                        if not is_active:
                            conn.close()
                            return Response({'error': 'Coordinator profile not found or inactive'}, status=status.HTTP_404_NOT_FOUND)
                        # For 'both' shift get all levels from M2M
                        if shift == 'both':
                            cur.execute(
                                """SELECT level_id FROM coordinator_coordinator_assigned_levels
                                   WHERE coordinator_id=(
                                     SELECT id FROM coordinator_coordinator
                                     WHERE employee_code=%s AND is_deleted=false LIMIT 1
                                   )""",
                                (username,)
                            )
                            level_ids = [r[0] for r in cur.fetchall()]
                            managed_levels = list(Level.objects.filter(id__in=level_ids))
                        elif level_id:
                            try:
                                managed_levels = [Level.objects.get(pk=level_id)]
                            except Level.DoesNotExist:
                                pass
                conn.close()
            except Exception as e:
                return Response({'error': f'Coordinator profile not found or inactive (DB: {e})'}, status=status.HTTP_404_NOT_FOUND)
        ...
        for attendance_id in attendance_ids:
            try:
                with transaction.atomic():
                    ...
                    allowed = False
                    classroom_level = attendance.classroom.grade.level if attendance.classroom and attendance.classroom.grade else None
                    coord_level = coordinator.level
                    if coordinator.shift == 'both':
                        if coordinator.assigned_levels.exists():
                            if classroom_level in coordinator.assigned_levels.all():
                                allowed = True
                        elif coord_level:
                            if coord_level == classroom_level:
                                allowed = True
                    else:
                        if coord_level == classroom_level:
                            allowed = True
                    ...
                    if not allowed:
                        failed_count += 1
                        errors.append(f"Attendance {attendance_id}: Access denied (coord_level={coord_level}, classroom_level={classroom_level})")
                        continue
...
def grant_backfill_permission(request):
    ...
    try:
        ...
        permission = AttendanceBackfillPermission.objects.create(
            ...
            date=date_obj,
            granted_to=teacher,
            granted_by=_db_user(request.user),
            reason=reason,
            deadline=deadline
        )
    ...
    try:
        ...
        if not user.is_superadmin():
            if user.role == 'admin':
                ...
                # Org admin, teachers, principals, coordinators — filter by org
                # _TokenUser has org_id from JWT even when .organization is None
                org_id = getattr(getattr(user, 'organization', None), 'pk', None) or getattr(user, 'org_id', None)
                if org_id:
                    attendances = attendances.filter(organization_id=org_id)
                else:
...
def get_delete_logs(request):
    ...
    try:
        ...
        if user.is_superuser or (hasattr(user, 'role') and user.role in ['superadmin', 'org_admin']):
            ...
            campus_id = getattr(campus, 'id', None) or getattr(user, 'campus_id', None)
            if campus_id:
                try:
                    from campus.models import Campus
...
def biometric_devices(request):
    org = getattr(request.user, 'organization', None)
    if not org:
        # TokenUser.organization is always None; resolve the real Organization from the
        # JWT org_id claim (set by OrganizationMiddleware) — same pattern as sibling views.
        from users.middleware import get_current_organization
        org = get_current_organization()

    if request.method == 'GET':
        devices = BiometricDevice.objects.filter(organization=org)
        serializer = BiometricDeviceSerializer(devices, many=True)
        return Response(serializer.data)

    if request.method == 'POST':
        # Scope: only principals (and superadmin) add devices. Org admins are view-only.
        role = getattr(request.user, 'role', '')
        if role not in ('principal', 'superadmin'):
            return Response(
                {'error': 'Only a principal can add a device.'},
                status=status.HTTP_403_FORBIDDEN,
            )

        # Friendly duplicate check (serial_number is globally unique) — avoids a raw
        # 500 from the DB unique constraint.
        serial = (request.data.get('serial_number') or '').strip()
        if serial and BiometricDevice._base_manager.filter(serial_number=serial).exists():
            existing = BiometricDevice._base_manager.filter(serial_number=serial).first()
            same_org = existing.organization_id == getattr(org, 'id', None)
            msg = (f"A device with serial number '{serial}' is already registered"
                   + ("." if same_org else " to another organization. Contact support."))
...
@permission_classes([IsAuthenticated])
def biometric_device_detail(request, device_id):
    org = getattr(request.user, 'organization', None)
    if not org:
        # TokenUser.organization is always None; resolve the real Organization from the
        # JWT org_id claim (set by OrganizationMiddleware) — same pattern as sibling views.
        from users.middleware import get_current_organization
        org = get_current_organization()
    device = get_object_or_404(BiometricDevice, id=device_id, organization=org)

    if request.method == 'GET':
        return Response(BiometricDeviceSerializer(device).data)

    # Scope: only principals (and superadmin) edit/delete devices. Org admins view-only.
    role = getattr(request.user, 'role', '')
    if role not in ('principal', 'superadmin'):
        return Response({'error': 'Only a principal can modify a device.'}, status=status.HTTP_403_FORBIDDEN)
...
@api_view(['GET', 'POST'])
@permission_classes([IsAuthenticated])
def biometric_mappings(request):
    org = getattr(request.user, 'organization', None)
    if not org:
        # TokenUser.organization is always None; resolve the real Organization from the
        # JWT org_id claim (set by OrganizationMiddleware) — same pattern as sibling views.
    ...
    if request.method == 'GET':
        ...
        qs = BiometricEmployeeMapping.objects.filter(organization=org).select_related('device', 'user', 'teacher')
        if device_id:
            qs = qs.filter(device_id=device_id)
        serializer = BiometricEmployeeMappingSerializer(qs, many=True)
        return Response(serializer.data)

    if request.method == 'POST':
        # Scope: only principals (and superadmin) link staff to devices. Org admins view-only.
        role = getattr(request.user, 'role', '')
        if role not in ('principal', 'superadmin'):
            return Response({'error': 'Only a principal can map staff to a device.'}, status=status.HTTP_403_FORBIDDEN)

        # Allow bulk create: accept list or single object
        many = isinstance(request.data, list)
        serializer = BiometricEmployeeMappingSerializer(data=request.data, many=many)
        if serializer.is_valid():
            if many:
                serializer.save(organization=org, created_by=_db_user(request.user))
            else:
                serializer.save(organization=org, created_by=_db_user(request.user))
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['PUT', 'DELETE'])
@permission_classes([IsAuthenticated])
def biometric_mapping_detail(request, mapping_id):
    org = getattr(request.user, 'organization', None)
    if not org:
        # TokenUser.organization is always None; resolve the real Organization from the
        # JWT org_id claim (set by OrganizationMiddleware) — same pattern as sibling views.
        from users.middleware import get_current_organization
        org = get_current_organization()
    mapping = get_object_or_404(BiometricEmployeeMapping, id=mapping_id, organization=org)

    # Scope: only principals (and superadmin) edit/remove mappings. Org admins view-only.
    role = getattr(request.user, 'role', '')
    if role not in ('principal', 'superadmin'):
        return Response({'error': 'Only a principal can modify a mapping.'}, status=status.HTTP_403_FORBIDDEN)

    if request.method == 'PUT':
        serializer = BiometricEmployeeMappingSerializer(mapping, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def biometric_unmapped_staff(request):
    ...
    mapped_user_ids = BiometricEmployeeMapping.objects.filter(
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def biometric_sync_users(request, device_id):
    """
    Principal-triggered: pull enrolled users for a FoxFace device from the platform,
    create their mappings and auto-link to staff. Returns counts.
    """
    org = getattr(request.user, 'organization', None)
    if not org:
        from users.middleware import get_current_organization
    ...
    role = getattr(request.user, 'role', '')
    if role not in ('principal', 'superadmin'):
        return Response({'error': 'Only a principal can sync device users.'}, status=status.HTTP_403_FORBIDDEN)

    device = get_object_or_404(BiometricDevice, id=device_id, organization=org)

    from .services.foxface_client import FoxFaceClient, FoxFaceConfigError
    from .services.biometric_core import upsert_device_user_mapping

    client = FoxFaceClient()
    try:
        users = client.get_device_users(device.serial_number)
    except FoxFaceConfigError as e:
        return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        return Response({'error': f'Platform fetch failed: {e}'}, status=status.HTTP_502_BAD_GATEWAY)

    seen = linked = 0
    for u in users:
        _, was_linked = upsert_device_user_mapping(device, u['device_user_id'], u.get('device_user_name'))
        seen += 1
        linked += 1 if was_linked else 0

    return Response({'enrolled_users': seen, 'linked': linked})


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def biometric_auto_link(request):
    """
    Principal-triggered: re-run auto-linking over all unlinked mappings in the org
    (match device id -> staff by biometric_id/username). Returns how many were linked.
    """
    org = getattr(request.user, 'organization', None)
    if not org:
        from users.middleware import get_current_organization
    ...
    role = getattr(request.user, 'role', '')
    if role not in ('principal', 'superadmin'):
        return Response({'error': 'Only a principal can run auto-link.'}, status=status.HTTP_403_FORBIDDEN)

    from .services.biometric_core import auto_link_placeholder_mappings
    linked = auto_link_placeholder_mappings(organization=org)
    return Response({'linked': linked})


# ─── Legacy ADMS Push Receiver (ZKTeco-protocol devices) ─────────────────────────
# device_type='zkteco' machines push here. FoxFace face devices use foxface_push().

from django.views.decorators.csrf import csrf_exempt

@csrf_exempt
def adms_push(request):
    """
    ADMS-protocol biometric machine (e.g. ZKTeco) yahan punch data bhejti hai.
    ...
    from coordinator.models import Coordinator
    from principals.models import Principal
    from .models import StaffAttendance, BiometricEmployeeMapping
    ...
    for line in lines:
        ...
        if not user_id or not timestamp_str:
            print(f"--- [BIOMETRIC DEBUG] Invalid line: {line} ---")
            continue
        ...
        try:
            ...
            print(f"--- [BIOMETRIC DEBUG] Timestamp error for user {user_id}: {e} ---")
            continue

        # 4-6. Identify staff, resolve timing, upsert attendance — all via the
        # shared device-agnostic core (same path the FoxFace pull uses).
        from .services.biometric_core import process_punch, resolve_device
        device_obj = resolve_device(sn) if sn else device
        ...
        if result != 'marked':
            print(f"--- [BIOMETRIC DEBUG] {result} for ID: {user_id} (SN: {sn}) ---")
            continue

        processed_count += 1
    
    print(f"--- [BIOMETRIC] Processed {processed_count} logs for SN: {sn} ---")
    return HttpResponse("OK")
...
def _is_org_admin(user):
    """
    True if the request user is an org-level admin.

    The JWT-stateless _TokenUser exposes only `.role` (no is_org_admin_role()
    method), so a bare `user.is_org_admin_role()` raises AttributeError and 500s.
    This works for both _TokenUser and a real DB User. Superadmin is treated as
    org-admin for visibility, matching the frontend's isOrgAdmin.
    """
    role = getattr(user, 'role', '') or ''
    return (
        role in ('org_admin', 'admin', 'superadmin')
...
def staff_attendance_list(request):
    ...
    user = request.user
    org  = getattr(user, 'organization', None)
    # For _TokenUser (JWT-stateless), org is None; use org_id for explicit filtering
    org_id = getattr(org, 'pk', None) or getattr(user, 'org_id', None)

    # Keep the local staff roster fresh: newly added teachers/coordinators/principals
    # are pulled from auth_db (+ campus enrichment from staff_db) on demand so they
    # appear here immediately, without waiting for the periodic sync_master_data run.
    try:
        from attendance.management.commands.sync_master_data import sync_staff_users
        sync_staff_users()
    except Exception:
        pass
    ...
    # Base staff QuerySet — manager auto-filters by org via context; exclude non-staff roles
    staff_qs = User.objects.filter(is_active=True).exclude(role__in=['student', 'superadmin', 'donor'])
    # Explicit org filter as fallback in case manager context is not set
    if org_id:
        staff_qs = staff_qs.filter(organization_id=org_id)

    # _TokenUser has campus_id (not campus FK); use campus_id throughout
    user_campus_id = getattr(user, 'campus_id', None)

    PRINCIPAL_VISIBLE_ROLES = ['teacher', 'coordinator', 'accounts_officer', 'admissions_counselor', 'compliance_officer']

    if user.is_principal():
        # Principal sees all staff at their campus (teachers + coordinators)
        staff_qs = staff_qs.filter(campus_id=user_campus_id, role__in=PRINCIPAL_VISIBLE_ROLES)
    elif user.is_coordinator():
        # Coordinator sees only the teachers directly assigned to them (via M2M)
        from django.db.models import Q
        from coordinator.models import Coordinator
        coordinator_obj = Coordinator.get_for_user(user)
        emp_codes, emails = _get_coordinator_teacher_identifiers(user, coordinator_obj)
        if emp_codes or emails:
            # User model uses `username` as employee_code
            staff_qs = staff_qs.filter(
                Q(username__in=emp_codes) | Q(email__in=emails),
                role='teacher',
            )
        else:
            # fallback: all teachers at campus
            staff_qs = staff_qs.filter(campus_id=user_campus_id, role='teacher')
    elif _is_org_admin(user):
        # Org Admin can filter by campus if provided
        if campus_id:
            staff_qs = staff_qs.filter(campus_id=campus_id, role='principal')
        else:
    ...
    for staff in staff_qs:
        ...
        if rec:
            ...
            data = {
                ...
                'staff_role': staff.role,  # raw role code, not display name
...
def staff_attendance_mark(request):
    ...
    coordinator_assigned_user_ids = None  # will be set for coordinator role
    if user.is_superuser or _is_org_admin(user):
        allowed_roles = ['principal']
    elif user.is_principal():
        allowed_roles = ['teacher', 'coordinator', 'accounts_officer', 'admissions_counselor', 'compliance_officer']
    elif user.is_coordinator():
        allowed_roles = ['teacher']
        # Pre-compute the set of teacher user IDs this coordinator may mark
        from django.db.models import Q
        from coordinator.models import Coordinator
        coordinator_obj = Coordinator.get_for_user(user)
        emp_codes, emails = _get_coordinator_teacher_identifiers(user, coordinator_obj)
        if emp_codes or emails:
            coordinator_assigned_user_ids = set(
                User.objects.filter(
                    Q(username__in=emp_codes) | Q(email__in=emails),
                    role='teacher',
                ).values_list('id', flat=True)
    ...
    # Resolve the marker as a real DB User (for marked_by FK)
    marker_db = None
    try:
        marker_db = User.objects.get(id=user.id)
    except Exception:
        pass
    ...
    for item in records:
        try:
            ...
            qs = User.objects.filter(id=target_user_id)
            if org_id:
                qs = qs.filter(organization_id=org_id)
            target_user = qs.get()

            # Security: role must be in allowed list
            ...
            # Security: campus check — compare IDs, not object references
            if not user.is_superuser and not _is_org_admin(user):
                if target_user.campus_id != user_campus_id:
                    errors.append(f"Not allowed to mark {target_user.get_full_name()} — different campus.")
                    continue

            # Security: coordinator can only mark their directly assigned teachers
            if user.is_coordinator() and coordinator_assigned_user_ids is not None:
                if target_user.id not in coordinator_assigned_user_ids:
                    errors.append(f"Not allowed to mark {target_user.get_full_name()} — not your assigned teacher.")
                    continue

            # Can't mark yourself
            if target_user.id == user.id:
                errors.append("You cannot mark your own attendance.")
                continue

            # Auto-calculate late arrival and early departure from shift timing
            raw_status = item.get('status', 'absent')
            check_in_str  = item.get('check_in_time') or None
            check_out_str = item.get('check_out_time') or None
            late_mins = 0
            early_mins = 0
            if raw_status in ('present', 'late') and (check_in_str or check_out_str):
                try:
                    shift_start, shift_end, grace = _resolve_shift_timing_for_user(target_user)
                    if check_in_str:
                        ci = datetime.strptime(check_in_str, '%H:%M').time()
                        late_diff = int((datetime.combine(target_date, ci) - datetime.combine(target_date, shift_start)).total_seconds() // 60)
                        if late_diff > grace:
                            raw_status = 'late'
                            late_mins = late_diff
                    if check_out_str and shift_end:
                        co = datetime.strptime(check_out_str, '%H:%M').time()
                        early_diff = int((datetime.combine(target_date, shift_end) - datetime.combine(target_date, co)).total_seconds() // 60)
                        if early_diff > 0:
                            early_mins = early_diff
                except Exception:
                    pass

            # Upsert — use org_id not org object to avoid NULL lookup
            rec, created = StaffAttendance.objects.update_or_create(
                organization_id=org_id,
                user=target_user,
                date=target_date,
                defaults={
                    'campus_id': target_user.campus_id,
                    'status': raw_status,
                    'late_minutes': late_mins,
                    'early_departure_minutes': early_mins,
                    'check_in_time': check_in_str or None,
                    'check_out_time': check_out_str or None,
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def staff_attendance_recalculate_late(request):
    """Recalculate late_minutes + status for existing 'present' records on a date.
    Useful after shift assignments change or to fix records saved before late-calc was added."""
    from users.models import User
    user = request.user
    org = getattr(user, 'organization', None)
    org_id = getattr(org, 'pk', None) or getattr(user, 'org_id', None)
    ...
    qs = StaffAttendance._base_manager.filter(date=target_date)
    if org_id:
        qs = qs.filter(organization_id=org_id)
    if user_campus_id and not (user.is_superuser or _is_org_admin(user)):
        qs = qs.filter(campus_id=user_campus_id)

    updated = 0
    for rec in qs.select_related('user'):
        if rec.status not in ('present', 'late', 'half_day'):
            continue
        shift_start, shift_end, grace = _resolve_shift_timing_for_user(rec.user)
        new_late = 0
        new_status = rec.status
        if rec.check_in_time:
            late_diff = int((datetime.combine(target_date, rec.check_in_time) - datetime.combine(target_date, shift_start)).total_seconds() // 60)
            if late_diff > grace:
                new_status = 'late'
                new_late = late_diff
                ...
                new_status = 'present'
        new_early = 0
        if rec.check_out_time and shift_end:
            early_diff = int((datetime.combine(target_date, shift_end) - datetime.combine(target_date, rec.check_out_time)).total_seconds() // 60)
            if early_diff > 0:
                new_early = early_diff
        if rec.status != new_status or rec.late_minutes != new_late or rec.early_departure_minutes != new_early:
            rec.status = new_status
            rec.late_minutes = new_late
            rec.early_departure_minutes = new_early
            rec.save(update_fields=['status', 'late_minutes', 'early_departure_minutes'])
            updated += 1
...
def staff_attendance_summary(request):
    ...
    # Base staff QuerySet — manager auto-filters by org via context
    staff_qs = User.objects.filter(is_active=True).exclude(role__in=['student', 'superadmin', 'donor'])
    if org_id:
        staff_qs = staff_qs.filter(organization_id=org_id)
    ...
    records = StaffAttendance.objects.filter(
        date=target_date, user__in=staff_qs
    )
...
def employee_timings_list(request):
    ...
    campus_id = request.query_params.get('campus_id')
    qs = User.objects.filter(is_active=True).exclude(role__in=['student', 'superadmin', 'donor'])
    if org_id:
        qs = qs.filter(organization_id=org_id)
    ...
    if user.is_principal():
        qs = qs.filter(campus_id=user_campus_id, role__in=PRINCIPAL_VISIBLE_ROLES)
    elif user.is_coordinator():
        qs = qs.filter(campus_id=user_campus_id, role='teacher')
    elif _is_org_admin(user):
        if campus_id:
            qs = qs.filter(campus_id=campus_id, role='principal')
        else:
            qs = qs.filter(role='principal')

    # Load campus default for fallback display
    from .models import CampusDefaultTiming
    campus_default = None
    try:
        campus_default = CampusDefaultTiming.objects.filter(
            campus_id=user_campus_id or campus_id, is_active=True
        ).first()
    except Exception:
        pass
    ...
    for u in qs.select_related('shift_timing', 'campus'):
        ...
        try:
            ...
            shift_type = timing.shift_type or 'custom'
        ...
        result.append({
            ...
            'timing_id': tid,
            'shift_type': shift_type,       # morning / afternoon / full_day / custom / None
            'check_in_time': ci,            # only meaningful when shift_type='custom'
            'check_out_time': co,
            'grace_minutes': grace,
        })
    return Response(result)
...
def employee_timings_bulk_save(request):
    ...
    if not (user.is_superuser or _is_org_admin(user) or
    ...
    for item in items:
        ...
        shift_type = item.get('shift_type') or 'custom'
        ci = item.get('check_in_time') or None
        co = item.get('check_out_time') or None
        grace = item.get('grace_minutes', 10)

        if not target_id:
            continue
        # For non-custom shifts, times come from campus timing — don't require them here
        if shift_type == 'custom' and (not ci or not co):
            continue

        try:
            qs = User.objects.filter(id=target_id)
            if org_id:
                qs = qs.filter(organization_id=org_id)
            target_user = qs.get()
            defaults = {
            ...
            if shift_type == 'custom':
                defaults['check_in_time'] = ci
                defaults['check_out_time'] = co
            else:
                defaults['check_in_time'] = None
                defaults['check_out_time'] = None
...
def _resolve_shift_timing_for_user(user_obj):
    """Return (shift_start, shift_end, grace_minutes) for a staff user.
    Cascade: employee custom → employee preset → campus full_day → hardcoded 08:00/17:00."""
    from .models import CampusDefaultTiming
    ...
    try:
        ...
        if emp_timing and emp_timing.is_active:
            if emp_timing.shift_type == 'custom' and emp_timing.check_in_time:
                shift_start = emp_timing.check_in_time
                shift_end   = emp_timing.check_out_time or time(17, 0)
                grace = emp_timing.grace_minutes
                resolved = True
            elif emp_timing.shift_type in ('morning', 'afternoon', 'full_day'):
                ...
                if campus_timing:
                    shift_start = campus_timing.check_in_time
                    shift_end   = campus_timing.check_out_time
                    grace = campus_timing.grace_minutes
                    resolved = True
    except Exception:
        pass
    if not resolved:
        try:
            ...
            pass
    return shift_start, shift_end, grace


def _shift_timing_dict(shift_type, timing=None):
    """Return a standardized shift timing dict."""
    DEFAULTS = {
        'morning':   {'check_in_time': '08:00', 'check_out_time': '12:00', 'grace_minutes': 10},
        'afternoon': {'check_in_time': '12:00', 'check_out_time': '17:00', 'grace_minutes': 10},
        'full_day':  {'check_in_time': '08:00', 'check_out_time': '17:00', 'grace_minutes': 10},
    }
    base = DEFAULTS.get(shift_type, {'check_in_time': None, 'check_out_time': None, 'grace_minutes': 10})
    if timing:
        return {
            'shift_type': shift_type,
            'check_in_time': str(timing.check_in_time)[:5],
            'check_out_time': str(timing.check_out_time)[:5],
            'grace_minutes': timing.grace_minutes,
            'is_set': True,
        }
    return {**base, 'shift_type': shift_type, 'is_set': False}


@api_view(['GET'])
@permission_classes([IsAuthenticated])
def campus_timing_get(request):
    """GET /api/attendance/campus-timing/ — return all shift configs for campus."""
    from .models import CampusDefaultTiming
    user = request.user
    org_id = getattr(getattr(user, 'organization', None), 'pk', None) or getattr(user, 'org_id', None)
    campus_id = getattr(user, 'campus_id', None) or request.query_params.get('campus_id')

    records = {
        r.shift_type: r
        for r in CampusDefaultTiming.objects.filter(campus_id=campus_id, organization_id=org_id, is_active=True)
    }
    return Response({
        'morning':   _shift_timing_dict('morning',   records.get('morning')),
        'afternoon': _shift_timing_dict('afternoon', records.get('afternoon')),
        'full_day':  _shift_timing_dict('full_day',  records.get('full_day')),
    })


@api_view(['POST'])
@permission_classes([IsAuthenticated])
def campus_timing_save(request):
    """POST /api/attendance/campus-timing/save/ — upsert one shift config."""
    from .models import CampusDefaultTiming
    from campus.models import Campus

    user = request.user
    if not (user.is_principal() or _is_org_admin(user) or user.is_superuser):
        return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)

    org_id = getattr(getattr(user, 'organization', None), 'pk', None) or getattr(user, 'org_id', None)
    campus_id = getattr(user, 'campus_id', None) or request.data.get('campus_id')
    shift_type = request.data.get('shift_type', 'full_day')
    ci = request.data.get('check_in_time')
    co = request.data.get('check_out_time')
    grace = request.data.get('grace_minutes', 10)

    if shift_type not in ('morning', 'afternoon', 'full_day'):
        return Response({'error': 'shift_type must be morning, afternoon, or full_day'}, status=status.HTTP_400_BAD_REQUEST)
    if not campus_id or not ci or not co:
        return Response({'error': 'campus_id, check_in_time, check_out_time are required'}, status=status.HTTP_400_BAD_REQUEST)

    try:
        campus = Campus.objects.get(pk=campus_id)
        obj, created = CampusDefaultTiming.objects.update_or_create(
            campus=campus, shift_type=shift_type,
            defaults={
                'organization_id': org_id,
                'check_in_time': ci,
                'check_out_time': co,
        ...
        return Response({'saved': True, 'shift_type': shift_type, 'created': created})
    except Exception as e:
