...
def sync_users():
    """Sync staff users from staff_db so teacher FK (user_id) is satisfied."""
    try:
        conn = _staff_conn()
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        logger.warning("sync_users: cannot reach staff DB: %s", e)
        return 0
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Sync users before teachers (teachers.user_id FK)
        self.stdout.write("Syncing users...")
        n = sync_users()
        self.stdout.write(f"  {n} users synced.")

        # Sync teachers before classrooms (classrooms.class_teacher_id FK)
