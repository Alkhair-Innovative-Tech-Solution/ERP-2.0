...
export default function TransferManagementPage() {
  ...
  // Teacher / Coordinator view: only class + shift transfers
    if (loading) {
      return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-white p-3 sm:p-5 md:p-8 pb-12">
          <div className="max-w-6xl mx-auto space-y-5 animate-pulse">
            {/* Header skeleton */}
            <div className="h-28 rounded-3xl bg-gradient-to-r from-gray-200 to-gray-300" />
            {/* Tabs skeleton */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-2">
              <div className="grid grid-cols-4 gap-1">
                {[1,2,3,4].map(i => <div key={i} className="h-11 rounded-xl bg-gray-100" />)}
              </div>
            </div>
            {/* Cards skeleton */}
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-3 flex justify-between">
              <div className="h-7 w-36 bg-gray-100 rounded-xl" />
              <div className="h-7 w-48 bg-gray-100 rounded-xl" />
            </div>
            <div className="grid gap-3">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex items-center gap-4">
  ...
  const campusIncoming = campusTransfers.filter(t => {
    ...
    if (isPrincipal && currentUserId) {
      return t.from_principal === currentUserId || t.to_principal === currentUserId;
    } else if (currentCoordinatorId) {
      return t.from_coordinator === currentCoordinatorId || t.to_coordinator === currentCoordinatorId;
    }
    return false;
  });
  ...
  // All users view (4 tabs: Class, Shift, Grade Skip, Campus)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-white p-3 sm:p-5 md:p-8 pb-12">
        <div className="max-w-6xl mx-auto space-y-5 sm:space-y-6">
          ...
          {/* Premium Header */}
          <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-r from-[#0A4174] via-[#0d5099] to-[#1468c8] p-5 sm:p-7 text-white shadow-2xl">
            ...
            <div className="absolute top-2 right-32 w-20 h-20 rounded-full bg-white/5 pointer-events-none" />
            <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3 sm:gap-4">
                <button
                  onClick={() => router.back()}
                  className="flex items-center gap-1.5 text-sm text-white/80 hover:text-white bg-white/10 hover:bg-white/20 px-3 py-2 rounded-xl transition-all font-medium shrink-0"
                >
                ...
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <ArrowRightLeft className="h-5 w-5 sm:h-6 sm:w-6 text-blue-200 shrink-0" />
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight">Transfer Management</h1>
                  </div>
                  <p className="text-blue-200/80 text-xs sm:text-sm">View and manage all transfer requests across your institution</p>
                </div>
              </div>
              {!isPrincipal && (
                <Button
                  onClick={() => router.push('/admin/principals/transfers/create')}
                  className="bg-white text-[#0A4174] hover:bg-blue-50 font-semibold shadow-lg rounded-xl gap-2 px-4 sm:px-5 h-10 text-sm shrink-0 w-full sm:w-auto"
                >
                  <Plus className="h-4 w-4" />
                  <span className="sm:hidden">Create Transfer</span>
        ...
        <Tabs defaultValue="class" className="space-y-4 sm:space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-1.5">
            <TabsList className="grid w-full grid-cols-4 bg-transparent gap-1 h-auto p-0">
              <TabsTrigger value="class" className="rounded-xl data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-all gap-1.5 data-[state=inactive]:text-gray-500 data-[state=inactive]:hover:bg-gray-50 h-auto">
                <GraduationCap className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
                <span className="hidden sm:inline">Class Transfers</span>
                <span className="sm:hidden">Class</span>
              </TabsTrigger>
              <TabsTrigger value="shift" className="rounded-xl data-[state=active]:bg-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-md py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-all gap-1.5 data-[state=inactive]:text-gray-500 data-[state=inactive]:hover:bg-gray-50 h-auto">
                <ArrowRightLeft className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
                <span className="hidden sm:inline">Shift Transfers</span>
                <span className="sm:hidden">Shift</span>
              </TabsTrigger>
              <TabsTrigger value="grade-skip" className="rounded-xl data-[state=active]:bg-purple-600 data-[state=active]:text-white data-[state=active]:shadow-md py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-all gap-1.5 data-[state=inactive]:text-gray-500 data-[state=inactive]:hover:bg-gray-50 h-auto">
                <GraduationCap className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
                <span className="hidden sm:inline">Grade Skip</span>
                <span className="sm:hidden">Skip</span>
              </TabsTrigger>
              <TabsTrigger value="campus" className="rounded-xl data-[state=active]:bg-emerald-600 data-[state=active]:text-white data-[state=active]:shadow-md py-2.5 sm:py-3 text-xs sm:text-sm font-medium transition-all gap-1.5 data-[state=inactive]:text-gray-500 data-[state=inactive]:hover:bg-gray-50 h-auto">
          ...
          <TabsContent value="class" className="space-y-3 sm:space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-blue-100 flex items-center justify-center">
                  <GraduationCap className="h-4 w-4 text-blue-600" />
                </div>
                <h2 className="text-sm sm:text-base font-semibold text-gray-800">Class Transfers</h2>
              </div>
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setClassDirectionFilter('incoming')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${classDirectionFilter === 'incoming' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Incoming
                  </button>
                  <button type="button" onClick={() => setClassDirectionFilter('outgoing')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${classDirectionFilter === 'outgoing' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Outgoing
                  </button>
                </div>
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setClassStatusFilter('pending')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${classStatusFilter === 'pending' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Pending <span className="ml-1 opacity-80">({pendingClassCount})</span>
                  </button>
                  <button type="button" onClick={() => setClassStatusFilter('history')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${classStatusFilter === 'history' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    History <span className="ml-1 opacity-80">({historyClassCount})</span>
                  </button>
                </div>
              </div>
            </div>
            {filteredClassTransfers.length === 0 ? (
              <div className="bg-white rounded-2xl border border-blue-100/50 shadow-sm py-14 px-6 text-center flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center shadow-inner">
                  <FileText className="h-7 w-7 text-blue-400" />
                </div>
                <p className="text-base font-semibold text-gray-800">
                  {classStatusFilter === 'pending' ? `No pending ${classDirectionFilter} class transfers` : `No ${classDirectionFilter} class transfer history yet`}
                </p>
                <p className="text-xs text-gray-400 max-w-sm">
                  {classStatusFilter === 'pending'
                    ? classDirectionFilter === 'incoming' ? 'No class/section transfer requests are waiting for your approval.' : 'You have not created any pending class transfer requests.'
                    : classDirectionFilter === 'incoming' ? 'Approved or declined transfers will appear here for your reference.' : 'Your processed transfer requests will appear here.'}
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
          ...
          <TabsContent value="shift" className="space-y-3 sm:space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm sm:text-base font-semibold text-gray-800">Shift Transfers</h2>
              </div>
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setShiftDirectionFilter('incoming')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${shiftDirectionFilter === 'incoming' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Incoming
                  </button>
                  <button type="button" onClick={() => setShiftDirectionFilter('outgoing')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${shiftDirectionFilter === 'outgoing' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Outgoing
                  </button>
                </div>
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setShiftStatusFilter('pending')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${shiftStatusFilter === 'pending' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Pending <span className="ml-1 opacity-80">({pendingShiftCount})</span>
                  </button>
                  <button type="button" onClick={() => setShiftStatusFilter('history')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${shiftStatusFilter === 'history' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    History <span className="ml-1 opacity-80">({historyShiftCount})</span>
                  </button>
                </div>
              </div>
            </div>
            {filteredShiftTransfers.length === 0 ? (
              <div className="bg-white rounded-2xl border border-indigo-100/50 shadow-sm py-14 px-6 text-center flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-50 to-indigo-100 flex items-center justify-center shadow-inner">
                  <ArrowRightLeft className="h-7 w-7 text-indigo-400" />
                </div>
                <p className="text-base font-semibold text-gray-800">
                  {shiftStatusFilter === 'pending' ? `No pending ${shiftDirectionFilter} shift transfers` : `No ${shiftDirectionFilter} shift transfer history yet`}
                </p>
                <p className="text-xs text-gray-400 max-w-sm">
                  {shiftStatusFilter === 'pending'
                    ? shiftDirectionFilter === 'incoming' ? 'No shift transfer requests are waiting for your approval.' : 'You have not created any pending shift transfer requests.'
                    : shiftDirectionFilter === 'incoming' ? 'Approved or declined shift transfers will appear here for your records.' : 'Your processed shift transfer requests will appear here.'}
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
          ...
          <TabsContent value="grade-skip" className="space-y-3 sm:space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm sm:text-base font-semibold text-gray-800">Grade Skip Transfers</h2>
              </div>
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setGradeSkipDirectionFilter('incoming')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${gradeSkipDirectionFilter === 'incoming' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Incoming
                  </button>
                  <button type="button" onClick={() => setGradeSkipDirectionFilter('outgoing')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${gradeSkipDirectionFilter === 'outgoing' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Outgoing
                  </button>
                </div>
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setGradeSkipStatusFilter('pending')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${gradeSkipStatusFilter === 'pending' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Pending
                  </button>
                  <button type="button" onClick={() => setGradeSkipStatusFilter('history')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${gradeSkipStatusFilter === 'history' ? 'bg-purple-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    History
                  </button>
                </div>
              </div>
            ...
            {filteredGradeSkipTransfers.length === 0 ? (
              <div className="bg-white rounded-2xl border border-purple-100/50 shadow-sm py-14 px-6 text-center flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-50 to-purple-100 flex items-center justify-center shadow-inner">
                  <GraduationCap className="h-7 w-7 text-purple-400" />
                </div>
                <p className="text-base font-semibold text-gray-800">
                  {gradeSkipStatusFilter === 'pending' ? `No pending ${gradeSkipDirectionFilter} grade skip transfers` : `No ${gradeSkipDirectionFilter} grade skip transfer history yet`}
                </p>
                <p className="text-xs text-gray-400 max-w-sm">
                  {gradeSkipStatusFilter === 'pending'
                    ? gradeSkipDirectionFilter === 'incoming' ? 'No grade skip requests are waiting for your approval.' : 'You have not created any pending grade skip requests.'
                    : gradeSkipDirectionFilter === 'incoming' ? 'Approved or declined grade skips will appear here.' : 'Your processed grade skip requests will appear here.'}
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
          ...
          <TabsContent value="campus" className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm sm:text-base font-semibold text-gray-800">Campus Transfers</h2>
              </div>
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setCampusDirectionFilter('incoming')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${campusDirectionFilter === 'incoming' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Incoming
                  </button>
                  <button type="button" onClick={() => setCampusDirectionFilter('outgoing')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${campusDirectionFilter === 'outgoing' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Outgoing
                  </button>
                </div>
                <div className="inline-flex rounded-xl bg-gray-100 p-1 text-xs gap-0.5">
                  <button type="button" onClick={() => setCampusStatusFilter('pending')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${campusStatusFilter === 'pending' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    Pending <span className="ml-1 opacity-80">({pendingCampusCount})</span>
                  </button>
                  <button type="button" onClick={() => setCampusStatusFilter('history')}
                    className={`px-3 py-1.5 rounded-lg transition-all font-medium text-xs ${campusStatusFilter === 'history' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    History <span className="ml-1 opacity-80">({historyCampusCount})</span>
                  </button>
                </div>
              </div>
            </div>
            {filteredCampusTransfers.length === 0 ? (
              <div className="bg-white rounded-2xl border border-emerald-100/50 shadow-sm py-14 px-6 text-center flex flex-col items-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100 flex items-center justify-center shadow-inner">
                  <Building className="h-7 w-7 text-emerald-400" />
                </div>
                <p className="text-base font-semibold text-gray-800">
                  {campusStatusFilter === 'pending' ? `No pending ${campusDirectionFilter} campus transfers` : `No ${campusDirectionFilter} campus transfer history yet`}
                </p>
                <p className="text-xs text-gray-400 max-w-sm">
                  {campusStatusFilter === 'pending'
                    ? campusDirectionFilter === 'incoming' ? 'There are currently no campus transfer requests waiting for your approval.' : 'You have not created any pending campus transfer requests.'
                    : campusDirectionFilter === 'incoming' ? 'Approved or declined campus transfers will be listed here for your records.' : 'Your approved or declined campus transfer requests will appear here.'}
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
