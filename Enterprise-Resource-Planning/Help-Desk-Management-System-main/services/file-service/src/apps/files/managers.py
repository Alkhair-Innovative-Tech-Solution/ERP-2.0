...
tenant-scoping queryset, so Attachment keeps its existing soft-delete
behavior (default-filtered to is_deleted=False) while gaining
`.for_tenant(tenant_id)`.

Lives in file-service, not central_auth or hdms_core — this composition is
specific to this service's own base model choice, not part of either shared
template (mirrors ticket-service's apps/tickets/models/managers.py from
Increment 2b, and communication-service's apps/chat/managers.py from 2c).
"""
