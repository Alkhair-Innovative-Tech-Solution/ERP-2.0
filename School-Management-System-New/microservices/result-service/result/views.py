...
def _get_principal_campus_id(user):
    ...
    from django.http import Http404
    raise Http404("Principal has no campus assigned.")
...
class PrincipalResultStatsView(APIView):
    ...
    def get(self, request):
        try:
            campus_id = _get_principal_campus_id(request.user)

            # Get all classrooms in the campus
            classrooms = ClassRoom.objects.filter(
                grade__level__campus_id=campus_id
...
class ResultViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            try:
                campus_id = _get_principal_campus_id(user)
                return Result.objects.filter(student__campus_id=campus_id).select_related('student', 'teacher', 'student__classroom')
            except Exception:
...
class PrincipalApproveResultView(APIView):
    ...
    def post(self, request, pk):
        ...
        result.status = 'approved'
        result.approved_by_principal = principal  # None if not synced — field is nullable
        ...
        if signature:
            ...
            if principal and not principal.signature:
    ...
    def post(self, request):
        ...
        if user_is_coordinator(user):
            ...
            try:
                ...
                return Response({'error': 'Principal campus not found'}, status=status.HTTP_404_NOT_FOUND)
        else:
...
class ResultsBulkUploadView(APIView):
    ...
    def post(self, request):
        ...
        teacher = Teacher.objects.filter(Q(email=request.user.email) | Q(employee_code=request.user.username)).first()
        if not teacher:
            return Response(
...
class SampleMonthlyTemplateView(TemplateDataMixin, APIView):
    ...
    def get(self, request):
        ...
        subject_labels = subjects[:10]

        # Build HTML table
        html = ['<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"/></head><body>']
        html.append('<table border="1"><tr>')
        # write non-subject headers (including total_marks)
        for h in headers[:7]:
            html.append(f'<th>{h}</th>')
...
class SampleMidTemplateView(TemplateDataMixin, APIView):
    ...
    def get(self, request):
        ...
        # students/teacher_id come from the shared mixin; subjects use campus hints below.
        _subjects, students, teacher_id, _month = self.get_template_data(request)
        headers = [
...
class AvailableSubjectsView(APIView):
    ...
    def get(self, request):
        ...
        if student_id:
            # Try local DB first; fall back to student-service API
            student = Student._base_manager.filter(id=student_id).first()
            if student:
                campus = getattr(student, 'campus_from_classroom', None) or getattr(student, 'campus', None)
                campus_id = getattr(campus, 'id', None) or getattr(campus, 'pk', None)
                level = getattr(student, 'level', None)
                level_id = getattr(level, 'id', None) if level else None
            else:
                # Fetch from student-service
                try:
                    student_url = os.environ.get('STUDENT_SERVICE_URL', 'http://student-service:8005')
                    sresp = req_lib.get(
                        f"{student_url}/api/students/{student_id}/",
                        headers={'Authorization': auth_header},
                        timeout=5,
                    )
                    if sresp.status_code == 200:
                        sdata = sresp.json()
                        campus_id = sdata.get('campus_id') or (sdata.get('campus', {}) or {}).get('id')
                        level_id = sdata.get('level_id') or (sdata.get('level', {}) or {}).get('id')
                    else:
                        return Response({'subjects': []})
                except Exception:
                    return Response({'subjects': []})

        if not campus_id and classroom_id:
            try:
                from classes.models import ClassRoom
        ...
        # Fetch subjects from timetable-service API
        timetable_url = os.environ.get('TIMETABLE_SERVICE_URL', 'http://timetable-service:8009')
        auth_header = request.headers.get('Authorization', '')
        params = {'campus': campus_id, 'is_active': 'true'}
        if level_id:
            params['level'] = level_id

        try:
            resp = req_lib.get(
                f"{timetable_url}/api/timetable/subjects/",
                params=params,
                headers={'Authorization': auth_header},
                timeout=5,
            )
            if resp.status_code == 200:
                resp_data = resp.json()
                raw = resp_data.get('results', resp_data) if isinstance(resp_data, dict) else resp_data
                if isinstance(raw, list):
                    # If level-specific subjects exist, prefer those; else return all campus subjects
                    if level_id:
                        level_subs = [s for s in raw if s.get('level') == level_id or
                                      (isinstance(s.get('level'), dict) and s['level'].get('id') == level_id)]
                        if level_subs:
                            raw = level_subs
                    data = [{'id': s['id'], 'name': s['name'], 'code': s.get('code', '')} for s in raw]
                    return Response({'subjects': sorted(data, key=lambda x: x['name'])})
        except Exception as e:
            pass
...
class PrincipalPerformanceDashboardView(OrgPerformanceDashboardView):
    ...
    def get(self, request):
        ...
        top_students = [
            {'name': r.student.name, 'grade': r.student.current_grade,
...
class CoordinatorPerformanceDashboardView(OrgPerformanceDashboardView):
    ...
    def get(self, request):
        ...
        top_students = [
            {'name': r.student.name, 'grade': r.student.current_grade, 'section': r.student.section,
...
class ClassTeacherPerformanceDashboardView(OrgPerformanceDashboardView):
    ...
    def get(self, request):
        ...
        for r in class_base.select_related('student').prefetch_related('subject_marks').order_by('-percentage'):
            ...
            row = {'student': r.student.name, 'subjects': {}}
...
class DonorPerformanceDashboardView(OrgPerformanceDashboardView):
    ...
    def get(self, request):
        ...
        inter_kpis = {
            'sponsored_lives': Student.objects.filter(is_active=True, is_deleted=False).count(),
