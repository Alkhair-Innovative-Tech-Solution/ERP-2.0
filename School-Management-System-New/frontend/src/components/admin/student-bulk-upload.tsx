...
import { UploadCloud, CheckCircle2, XCircle, FileSpreadsheet, Download, Loader2, RefreshCw, Info, Eye, Clock } from 'lucide-react'
          ...
          <CardContent>
            {!reports ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-8 text-center space-y-4 animate-in zoom-in duration-500">
                ...
                {elapsedSeconds != null && (
                  <div className="flex items-center gap-2 text-[#013a63] bg-[#013a63]/5 border border-[#013a63]/15 px-4 py-2 rounded-xl text-sm font-semibold">
                    <Clock className="w-4 h-4" />
                    {(reports?.length ?? 0)} record{(reports?.length ?? 0) === 1 ? '' : 's'} in {elapsedSeconds < 60
