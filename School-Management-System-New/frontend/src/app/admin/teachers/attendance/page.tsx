...
import { getCurrentUserProfile, getClassStudents, markBulkAttendance, getAttendanceHistory, getAttendanceForDate, editAttendance, submitAttendance, getBackfillPermissions, finalizeAttendance, coordinatorApproveAttendance, getApiBaseUrl, ApiError, getHolidays, getTeacherMyClasses } from "@/lib/api";
...
function TeacherAttendanceContent() {
  ...
  const fetchTeacherData = async () => {
    try {
      ...
      // Try my-classes API first (reliable — uses JWT username claim)
      let options: SimpleClassRoom[] = []
      try {
        const myClassesData = await getTeacherMyClasses()
        const assignments: any[] = myClassesData?.assignments || []
        if (assignments.length > 0) {
          const seen = new Set<number>()
          for (const a of assignments) {
            if (a.classroom_id && !seen.has(a.classroom_id)) {
      ...
      // Fallback to profile if my-classes returned nothing
      if (options.length === 0) {
        const multi = Array.isArray(teacherProfile.assigned_classrooms) ? teacherProfile.assigned_classrooms : []
        options = multi.length > 0 ? multi : (teacherProfile.assigned_classroom ? [teacherProfile.assigned_classroom] : [])
      }
      ...
      setClassInfo({
        ...
        code: selected.code || "",
        grade: typeof (selected as any).grade === 'string' ? (selected as any).grade : (selected as any).grade?.name || "",
                  ...
                  <Table>
                    ...
                    <TableHeader className="sticky top-0 z-10 bg-white shadow-[0_1px_0_0_rgb(229_231_235)]">
                      <TableRow>
                        ...
                        <TableHead className="text-xs sm:text-sm">Yesterday&apos;s Status</TableHead>
                        <TableHead className="text-xs sm:text-sm">Attendance %</TableHead>
                        <TableHead className="text-xs sm:text-sm">Consecutive Absence</TableHead>
                        <TableHead className="text-xs sm:text-sm">Today&apos;s Status</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredStudents.map((student) => {
