  ...
  const loadCampuses = async () => {
    try {
      ...
      if (userRole === 'principal' && user?.campus_id) {
        // Principal: only show their assigned campus and auto-select it
        ...
        onInputChange("campus", campusId)

        // Auto-select shift if campus has only one shift
        const campus = principalCampus[0]
        ...
        if (shiftAvailable !== 'both') {
          ...
          await loadGrades(campusId, formData.shift || '')
        }
      } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select 
              ...
              onValueChange={(v) => { 
                onInputChange("campus", v); 
                loadGrades(v, formData.shift);
                
                const campus = campuses.find(c => String(c.id) === String(v));
                const shiftAvailable = campus?.shift_available || 'both';
          ...
          <div>
            ...
            <Select value={formData.shift || ""} onValueChange={(v) => onInputChange("shift", v)}>
              ...
              <SelectContent>
                {(() => {
                  const campus = campuses.find(c => String(c.id) === String(formData.campus));
                  const shiftAvailable = campus?.shift_available || 'both';
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (!e.target.value || val <= new Date().getFullYear()) {
          ...
          <div className="md:col-span-2">
            <Label htmlFor="classroom" className="flex items-center gap-1">
              ...
              {formData.campus && formData.currentGrade && formData.section && formData.shift && (
                <button
                  type="button"
                  onClick={() => loadClassrooms(formData.campus, formData.currentGrade, formData.section, formData.shift)}
                  className="ml-auto flex items-center gap-1 text-[10px] text-blue-600 hover:text-blue-800"
