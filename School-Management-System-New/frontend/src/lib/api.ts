...
export function getApiBaseUrl(): string {
  ...
  if (typeof window === 'undefined') {
    return process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost';
  }
...
export async function authorizedFetch(path: string, init: RequestInit = {}, alreadyRetried = false): Promise<Response> {
  ...
  try {
    ...
    if (err?.name === 'AbortError') {
      throw new ApiError('Request timed out. Please try again.', 504, 'Timeout');
    }
    ...
    const buildQuery = () => {
      ...
      const appendMulti = (key: string, value?: any) => {
        ...
        if (Array.isArray(value)) {
          const validValues = value.filter(v => v !== undefined && v !== null && String(v).trim() !== '');
          if (validValues.length > 0) {
            qp.append(key, validValues.join(','));
          }
        } else {
...
export async function getCoordinatorTeachers(coordinatorId?: number) {
  try {
    // If no ID, use the self-lookup endpoint (works without coordinator_id in JWT)
    const url = coordinatorId
      ? `${API_ENDPOINTS.COORDINATORS}${coordinatorId}/teachers/`
      : `${API_ENDPOINTS.COORDINATORS}my-teachers/`;
...
export async function assignCoordinatorToLevel(levelId: number, coordinatorId: number, coordinatorName?: string, coordinatorCode?: string) {
  try {
...
export async function getCampusDefaultTiming(campusId?: number | string) {
  try {
    const params = new URLSearchParams();
    if (campusId) params.append('campus_id', String(campusId));
    return await apiGet(`/api/attendance/campus-timing/?${params.toString()}`);
  } catch (error) {
    console.error('Failed to fetch campus timing:', error);
    return {
...
export async function saveCampusDefaultTiming(data: { shift_type: string; check_in_time: string; check_out_time: string; grace_minutes: number; campus_id?: number | string }) {
  try {
    return await apiPost('/api/attendance/campus-timing/save/', data);
  } catch (error) {
    console.error('Failed to save campus timing:', error);
    throw error;
  }
}
...
export async function applyShiftToClassrooms(data: {
  campus_id: number;
  shift: string;
  ...
  return apiPost(`${API_ENDPOINTS.TIMETABLE}shift-timings/apply-to-classrooms/`, data);
}
...
// ─── Biometric Device & Mapping APIs ────────────────────────────────────────────

export async function getBiometricDevices(): Promise<any[]> {
  const res = await authorizedFetch('/api/attendance/biometric/devices/');
  if (!res.ok) throw new ApiError('Failed to fetch devices', res.status, res.statusText);
  return res.json();
}

export async function createBiometricDevice(data: any): Promise<any> {
  const res = await authorizedFetch('/api/attendance/biometric/devices/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getBiometricMappings(deviceId?: number): Promise<any[]> {
  const url = deviceId
    ? `/api/attendance/biometric/mappings/?device_id=${deviceId}`
    : '/api/attendance/biometric/mappings/';
  const res = await authorizedFetch(url);
  if (!res.ok) throw new ApiError('Failed to fetch mappings', res.status, res.statusText);
  return res.json();
}

export async function getBiometricUnmappedStaff(deviceId?: number): Promise<any[]> {
  const url = deviceId
    ? `/api/attendance/biometric/unmapped-staff/?device_id=${deviceId}`
    : '/api/attendance/biometric/unmapped-staff/';
  const res = await authorizedFetch(url);
  if (!res.ok) throw new ApiError('Failed to fetch unmapped staff', res.status, res.statusText);
  return res.json();
}

export async function createBiometricMapping(data: {
  device: number; device_user_id: string; device_user_name: string; user: number;
}): Promise<any> {
  const res = await authorizedFetch('/api/attendance/biometric/mappings/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
export async function updateBiometricMapping(mappingId: number, data: { user: number | null }): Promise<any> {
  const res = await authorizedFetch(`/api/attendance/biometric/mappings/${mappingId}/`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
export async function deleteBiometricMapping(mappingId: number): Promise<void> {
  const res = await authorizedFetch(`/api/attendance/biometric/mappings/${mappingId}/`, {
    method: 'DELETE',
  });
...
export async function syncBiometricDeviceUsers(deviceId: number): Promise<{ enrolled_users: number; linked: number }> {
  const res = await authorizedFetch(`/api/attendance/biometric/devices/${deviceId}/sync-users/`, { method: 'POST' });
  const json = await res.json();
  if (!res.ok) throw new ApiError(json.error || 'Failed to sync device users', res.status, res.statusText);
  return json;
}

export async function autoLinkBiometricMappings(): Promise<{ linked: number }> {
  const res = await authorizedFetch('/api/attendance/biometric/auto-link/', { method: 'POST' });
  const json = await res.json();
  if (!res.ok) throw new ApiError(json.error || 'Failed to auto-link', res.status, res.statusText);
  return json;
}
...
export async function recalculateStaffLate(date: string): Promise<{ recalculated: number }> {
  const res = await authorizedFetch('/api/attendance/staff/recalculate-late/', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ date }),
  });
  if (!res.ok) throw new ApiError('Failed to recalculate late status', res.status, res.statusText);
  return res.json();
}
