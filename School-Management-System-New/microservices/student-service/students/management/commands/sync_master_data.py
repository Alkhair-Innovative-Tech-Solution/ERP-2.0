...
def sync_orgs():
    ...
    # Skip FK columns and service-specific columns that should not be overwritten from org-service
...
def sync_classrooms():
    try:
        ...
        return 0
    # Skip user FK columns — users table is not synced to student-service
    skip_cols = {'assigned_by_id', 'class_teacher_id'}
    col_names = [c for c in col_names if c not in skip_cols]
    n = _upsert_table(rows, col_names, 'classes_classroom')

    # Backfill class_teacher_id from staff DB (campus DB has no teacher data)
    try:
        conn = _src_conn(
...
def sync_teachers():
    """Sync teacher records from staff-service so student-service can filter by teacher."""
    try:
        conn = _staff_conn()
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        return 0
    # Skip user_id — users are not synced to student-service
    skip_cols = {'user_id'}
    col_names = [c for c in col_names if c not in skip_cols]
...
class Command(BaseCommand):
    help = "Sync orgs, campuses, levels, grades, classrooms, and teachers from source DBs on startup"
