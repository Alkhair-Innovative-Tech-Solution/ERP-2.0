...
export function WorkAssignmentStep({ formData, invalidFields, onInputChange, campuses, formOptions }: WorkAssignmentStepProps) {
  const selectedCampus = campuses.find((c: any) => c.id === formData.campus || c.id === parseInt(formData.campus))
  const campusShift = selectedCampus?.shift_available

  const availableShifts = (() => {
    if (!campusShift) return []
    if (campusShift === 'morning') return [{ value: 'morning', label: 'Morning' }]
    if (campusShift === 'afternoon') return [{ value: 'afternoon', label: 'Afternoon' }]
    // 'both' means campus runs morning + afternoon
    return [
