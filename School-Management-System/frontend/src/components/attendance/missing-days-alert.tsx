...
export default function MissingDaysAlert({
  ...
  const remind = async (row: MissingDay) => {
    ...
    try {
      const result = await remindTeacher(row.classroom_id, row.dates);
  ...
  const remindAll = async () => {
    ...
    try {
      const result = await remindAllTeachers(range);
  ...
  if (dismissed || missingDays.length === 0) return null;

  const classCount = missingDays.length;
  const dayCount = missingDays.reduce((sum, m) => sum + m.dates.length, 0);
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase text-amber-800/70">
                    ...
                    <th className="pb-2 pr-4 font-medium">Missing dates</th>
